...
pub fn decrypt_password(encrypted_data: &str, master_password: &str) -> Result<String, EncryptionError> {
    ...
    if combined.len() < 12 {
        tracing::error!("Invalid nonce size: expected at least 12 bytes, got {}", combined.len());
...
mod tests {
    ...
    fn test_decrypt_with_wrong_key() {
        ...
        matches!(result.unwrap_err(), EncryptionError::PasswordDecryptionFailed);
    }
